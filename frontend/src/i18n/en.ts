export default {
  nav: {
    createRoom: 'Create Room',
    joinRoom: 'Join Room',
  },
  roomLogin: {
    title: 'Welcome, adventurer',
    createRoom: 'Create Room',
    joinRoom: 'Join Room',
    createDesc: 'Start a new scheduling quest',
    joinDesc: 'Enter an existing room',
    back: 'Back',
    roomName: 'Room name',
    password: 'Password',
    roomCode: 'Room code',
    create: 'Create',
    creating: 'Creating...',
    join: 'Join',
    next: 'Next',
    description: 'Description',
    descriptionPlaceholder: 'e.g. Friday night one-shot',
    minDuration: 'Min hours',
    maxDuration: 'Max hours',
    joinHint: "Tip: it's easier if you just click the link your friend shared!",
    availableTimesTitle: 'Session time window',
    availableTimesHint: 'What hours can sessions be scheduled?',
    availableFrom: 'From',
    availableTo: 'To',
    timezone: 'Timezone',
    roomAlreadyExists: 'A room with that name already exists',
    nameRequired: 'Room name is required',
    nameTooShort: 'Room name must be at least 3 characters',
    nameTooLong: 'Room name must be at most 100 characters',
    nameInvalidChars: 'Room name can only contain letters, numbers, and spaces',
    passwordRequired: 'Password is required',
    passwordTooShort: 'Password must be at least 3 characters',
    descriptionTooLong: 'Description must be at most 500 characters',
    day_monday: 'Mon',
    day_tuesday: 'Tue',
    day_wednesday: 'Wed',
    day_thursday: 'Thu',
    day_friday: 'Fri',
    day_saturday: 'Sat',
    day_sunday: 'Sun',
  },
  room: {
    loggingIn: 'Entering the room...',
    nameYourself: 'Name yourself',
    nameYourselfHint: 'How should the party know you?',
    yourName: 'Your name',
    namePlaceholder: 'e.g. Gandalf',
    saving: 'Saving...',
    continue: 'Continue',
    addPlayers: 'Add players',
    addPlayersHint: 'Invite the rest of the party (you can add more later)',
    playerName: 'Player name',
    playerNamePlaceholder: 'e.g. Legolas',
    add: 'Add',
    done: 'Done',
    admin: 'Admin',
    calendarTitle: 'Availability calendar',
    calendarPlaceholder: 'The calendar will appear here soon...',
    tabWeekly: 'Weekly',
    tabOverrides: 'Overrides',
    noDateSelected: 'Pick a date to edit',
    saved: 'Saved',
    saveError: 'Could not save',
    loadError: 'Could not load room',
    clipboardError: 'Could not copy to clipboard',
    weeklySubtitle: 'Select your available times',
    weeklyInfo:
      "Drag on the grid to paint the times you're free. You can drag across multiple days at once.",
    overridesSubtitle: 'Override specific dates',
    overridesInfo:
      'Pick a date from the calendar to block or unblock time slots for that day only, overriding your weekly schedule.',
    navUsers: 'Party',
    navCalendar: 'Calendar',
    navSettings: 'Settings',
    whoAreYou: 'Who are you?',
    whoAreYouHint: 'Pick your character from the party',
    you: 'You',
    settingsCopyLink: 'Copy share link',
    settingsCopyLinkHint: 'Anyone with this link will be able to join the room',
    settingsLinkCopied: 'Link copied!',
    settingsTheme: 'Theme',
    settingsLang: 'Language',
    settingsDuration: 'Session duration',
    settingsDurationMin: 'Min',
    settingsDurationMax: 'Max',
    settingsDurationUnit: 'h',
    durationCancel: 'Cancel',
    durationSave: 'Save',
    pin: 'PIN',
    enterPin: 'Enter PIN',
    setPin: 'Set PIN',
    changePin: 'Change PIN',
    removePin: 'Remove PIN',
    pinRequired: 'This user is PIN-protected',
    pinHint: 'Choose a 4-digit PIN to protect your avatar',
    wrongPin: 'Wrong PIN',
    pinSet: 'PIN set!',
    pinRemoved: 'PIN removed',
    removePinConfirm: 'Remove the PIN for {name}? They will be able to log in without a PIN.',
    adminPinRequired: 'As the admin, you must set a PIN',
    skipPin: 'Skip',
    skipPinTitle: 'Skip PIN setup',
    skipPinMessage:
      'Without a PIN, anyone in the room can select your avatar and modify your availability. Are you sure?',
    skipPinConfirm: 'Skip',
    pinSkippedWarning: 'Your avatar has no PIN — anyone can use it',
    logout: 'Switch user',
    resetAvailability: 'Reset my availability',
    resetAvailabilityHint: 'Clear all your painted times and overrides',
    resetAvailabilityDone: 'Availability reset!',
    editTimeWindow: 'Session time window',
    editTimeWindowHint: 'Set which hours are available for sessions each day',
    resetConfirmTitle: 'Reset availability',
    resetConfirmMessage:
      'This will clear all your painted times and overrides. This action cannot be undone.',
    resetConfirmButton: 'Reset',
    combinedCalendar: 'Combined Calendar',
    tabCombined: 'Combined',
    combinedSubtitle: 'Party availability overview',
    combinedInfo:
      'Shows when everyone is free for the selected week. Highlighted blocks are viable session times where all players are available for the minimum session duration.',
    viable: 'Viable session',
    all_free: 'All free',
    some_free: 'Some free',
    none: 'None',
    timezone: 'Timezone',
    changeTimezone: 'Change timezone',
    noSessionHours: 'No session hours',
    searchTimezone: 'Search timezone...',
    timezoneUpdated: 'Timezone updated',
    timeWindowTimezone: 'Times in {tz}',
    extendRoom: 'Extend room life',
    extendRoomHint: 'Reset the 30-day expiry timer',
    extendRoomDone: 'Room life extended!',
    deleteRoom: 'Delete room',
    deleteRoomHint: 'Permanently delete this room and all its data',
    deleteRoomConfirmTitle: 'Delete room',
    deleteRoomConfirmMessage:
      'This will permanently delete the room, all players, and all availability data. This action cannot be undone.',
    deleteRoomConfirmButton: 'Delete',
    confirm: 'Confirm',
    edit: 'Edit',
    editUser: 'Edit {name}',
    removeUser: 'Remove user',
    removeUserConfirm: 'Remove {name} from the party? This cannot be undone.',
    newName: 'New name',
    rename: 'Rename',
    makeAdmin: 'Make admin',
    makeAdminConfirm: 'Give {name} admin powers? They must have a PIN.',
    removeAdmin: 'Remove admin',
    removeAdminConfirm: 'Remove admin powers from {name}?',
    makeAdminRequiresPin: 'User must have a PIN to become admin',
  },
  feedback: {
    buttonLabel: 'Send feedback',
    title: 'Feedback & Suggestions',
    message: 'Your message',
    messagePlaceholder: 'Tell us what you think...',
    messageHint: 'At least 10 characters',
    name: 'Name (optional)',
    namePlaceholder: 'Anonymous',
    email: 'Email (optional)',
    emailPlaceholder: "your{'@'}email.com",
    submit: 'Send',
    submitting: 'Sending...',
    success: 'Thank you for your feedback!',
    error: 'Could not send feedback. Please try again.',
  },
  notFound: {
    message: 'This page does not exist.',
    home: 'Back to home',
  },
  home: {
    q1: 'Scheduling is hard?',
    q2: "Your friends can't communicate?",
    q3: 'Different timezones are a mess?',
    cta: 'Roll the Day20 to fix everything.',
    createRoom: 'Create Room',
    howItWorks: 'How it works',
    step1: 'Create a room and share it with your group',
    step2: 'Each player paints their weekly availability',
    step3: 'The app finds the best time for everyone',
    faq: 'Got questions?',
    faq1q: 'Is it free?',
    faq1a: 'Completely free. No account, no subscription, no limits.',
    faq2q: 'Do I need an account?',
    faq2a: 'Nope. Create a room with a name and password, share the link, done.',
    faq3q: 'What about different timezones?',
    faq3a:
      'Covered. Each player sets their own timezone and everyone sees availability in their local time.',
    faq4q: 'Only for D&D?',
    faq4a:
      'Not at all. Any group that needs to find a common time: tabletop RPGs, book clubs, game nights, online meetups, even work meetings. You name it.',
    faq5q: 'Are rooms kept forever?',
    faq5a: 'Nope. Rooms are deleted after 30 days of no activity, so no hoarding old campaigns.',
    demoRestart: 'Try again',
  },
}
